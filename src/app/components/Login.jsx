  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-8 lg:p-16">
        ...
        >
          <h1 className="text-4xl font-bold text-gray-800 mb-2">{dictionary?.title || "Welcome Back"}</h1>
          <p className="text-gray-600 mb-8">{dictionary?.description || "Login to your account"}</p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <Controller
              ...
              control={control}
              rules={{
                required: dictionary?.form?.inputs?.email?.errors?.required || 'Email is required',
                pattern: {
                  value: /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i,
                  message: dictionary?.form?.inputs?.email?.errors?.email || 'Invalid email address'
                }
              }}
              render={({ field }) => (
                <div>
            ...
            <Controller
              name="password"
              control={control}
              rules={{ required: dictionary?.form?.inputs?.password?.errors?.required || 'Password is required' }}
              render={({ field }) => (
                <div>
      ...
      <div className="hidden lg:block w-1/2 bg-[#aa9270] relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 text-white z-40">
          ...
          <p>Connect, collaborate, and grow with our community of professionals and clients.</p>
        </div>
      </div>
    </div>
  )
